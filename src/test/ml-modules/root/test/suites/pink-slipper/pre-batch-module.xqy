...
declare variable $pre-batch-mod-new-uri := "/pre-batch-module-test-new-value.xml"; (: the URI the pre-batch module will use to insert the new doc :)
declare variable $pre-batch-mod-new-val := sem:uuid-string(); (: generate a UUID - the selector module will throw an error if it's not there :) (: TODO: this comment is wrong :)
...
let $job-id := xdmp:eval(
  ...
  declare variable $tu:pre-batch-mod-new-uri as xs:string external;
  declare variable $tu:pre-batch-mod-new-val as xs:string external;
  ps:run(
    map:map(
      <map:map xmlns:map="http://marklogic.com/xdmp/map" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema">
        ...
        <map:entry key="PRE-BATCH-MODULE.pre-batch-mod-new-uri">
          <map:value xsi:type="xs:string">{$tu:pre-batch-mod-new-uri}</map:value>
        </map:entry>
        <map:entry key="PRE-BATCH-MODULE.pre-batch-mod-new-val">
          <map:value xsi:type="xs:string">{$tu:pre-batch-mod-new-val}</map:value>
        </map:entry>
        <map:entry key="PROCESS-MODULE">
          <map:value xsi:type="xs:string">{$tu:client-module-base-path || "/process.xqy"}</map:value>
        </map:entry>
        <map:entry key="PROCESS-MODULE.pre-batch-mod-new-uri">
          <map:value xsi:type="xs:string">{$tu:pre-batch-mod-new-uri}</map:value>
        </map:entry>
        <map:entry key="PROCESS-MODULE.pre-batch-mod-new-val">
          <map:value xsi:type="xs:string">{$tu:pre-batch-mod-new-val}</map:value>
        </map:entry>
      </map:map>
    )
  ...
  (
    ...
    xs:QName("tu:pre-batch-mod-new-uri"), $pre-batch-mod-new-uri,
    xs:QName("tu:pre-batch-mod-new-val"), $pre-batch-mod-new-val
  ),
...
return (
  ...
  (: assert pre-batch module status is successful :)
  test:assert-equal($ps:module-status-successful, ps:get-pre-batch-status($job-id)),
  ...
  (: assert the document created by the pre-batch mod exists and is correct :)
  let $pre-batch-mod-doc := fn:doc($pre-batch-mod-new-uri)
  let $pre-batch-mod-val := $pre-batch-mod-doc/val/fn:string()
  return (
    test:assert-exists($pre-batch-mod-doc),
    test:assert-equal($pre-batch-mod-new-val, $pre-batch-mod-val)
  ),
